...
fn insert_tx_into_table_sql(table_name: &str) -> Result<String, MmError<SqlError>> {
    ...
    let sql = "INSERT INTO ".to_owned()
        + table_name
        + " (tx_hash, tx_hex, internal_id, block_height, confirmation_status, token_id, details_json) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7);";
...
fn get_tx_hex_by_hash_sql(table_name: &str) -> Result<String, MmError<SqlError>> {
    validate_table_name(table_name)?;
...
impl TxHistoryStorage for SqliteTxHistoryStorage {
    ...
    async fn get_raw_tx_bytes(
        ...
        let sql = get_tx_hex_by_hash_sql(collection_id)?;
        let params = [tx_hash.to_owned()];
        let selfi = self.clone();
        async_blocking(move || {
            let conn = selfi.0.lock().unwrap();
...
mod sql_tx_history_storage_tests {
    ...
    fn test_remove_transaction() {
        ...
        block_on(storage.add_transactions(collection, [json::from_str(tx_json).unwrap()])).unwrap();
    ...
    fn test_update_transaction() {
        ...
        block_on(storage.add_transactions(collection, [tx_details.clone()])).unwrap();
    ...
    fn test_has_transactions_with_hash() {
        ...
        block_on(storage.add_transactions(collection, [tx_details])).unwrap();
    ...
    #[test]
    fn test_get_raw_tx_bytes() {
        let collection = "test_collection_for_get_raw_tx_bytes";
        let storage = SqliteTxHistoryStorage::in_memory();
        block_on(storage.init_collection(collection)).unwrap();

        let tx_hash = "2c33baf0c40eebcb70fc22eab0158e315e2176e4a3f20acddcd849186fca492c";

        let maybe_tx_hex = block_on(storage.get_raw_tx_bytes(collection, tx_hash)).unwrap();
        ...
        let expected_tx_hex = tx1.tx_hex.0.clone();

        let transactions = [tx1, tx2];
        block_on(storage.add_transactions(collection, transactions)).unwrap();

        let tx_hex = block_on(storage.get_raw_tx_bytes(collection, tx_hash))
            .unwrap()
            .unwrap();
