...
use web3::{ Error as Web3Error, ErrorKind as Web3ErrorKind, self, Web3 };
...
/// Version 1 of EtomicSwap smart contract is deployed to following addresses:
/// Dev chain (195.201.0.6:8565): 0xa09ad3cd7e96586ebd05a2607ee56b56fb2db8fd
...
lazy_static! {
    static ref SWAP_CONTRACT_V1: Contract = unwrap!(Contract::load(SWAP_CONTRACT_ABI_V1.as_bytes()));

    static ref SWAP_CONTRACT_V2: Contract = unwrap!(Contract::load(SWAP_CONTRACT_ABI_V2.as_bytes()));

    static ref VERSION_CONTRACT: Contract = unwrap!(Contract::load(VERSION_ABI.as_bytes()));
...
impl EthCoinImpl {
    ...
    fn eth_traces(
        ...
        to_block: BlockNumber,
        limit: Option<usize>
    ) -> impl Future<Item=Vec<Trace>, Error=String> {
        ...
        self.web3.trace().filter(filter.build()).map_err(|e| ERRL!("{}", e))
    }
    ...
    async fn search_for_swap_tx_spend(
        ...
        if let Some(event) = found {
            match event.transaction_hash {
                Some(tx_hash) => {
                    let transaction = match try_s!(self.web3.eth().transaction(TransactionId::Hash(tx_hash)).compat().await) {
                        Some(t) => t,
...
impl MarketCoinOps for EthCoin {
    ...
    fn wait_for_tx_spend(&self, tx_bytes: &[u8], wait_until: u64, from_block: u64) -> TransactionFut {
        ...
        let payment_func = try_fus!(SWAP_CONTRACT_V2.function(func_name));
...
impl EthCoin {
    ...
    fn send_hash_time_locked_payment(
        ...
        let fut = async move {
            let version = try_s!(coin.get_contract_version(coin.swap_contract_address).compat().await);
            match coin.coin_type {
                EthCoinType::Eth => {
                    let data = match version.as_u64() {
                        1 => {
                            let function = try_s!(SWAP_CONTRACT_V1.function("ethPayment"));
                            try_s!(function.encode_input(&[
                                Token::FixedBytes(id),
                        ...
                        2 => {
                            let function = try_s!(SWAP_CONTRACT_V2.function("ethPayment"));
                            try_s!(function.encode_input(&[
                                Token::FixedBytes(id),
                                Token::Address(receiver_addr),
                                Token::Uint(U256::from(time_lock)),
                                Token::Uint(secret_hash_algo),
                                Token::Bytes(secret_hash),
                            ]))
                        },
                        _ => return ERR!("Unsupported swap contract version {}", version),
                    };
                    coin.sign_and_send_transaction(value, Action::Call(coin.swap_contract_address), data, U256::from(150000)).compat().await
                },
                EthCoinType::Erc20(token_addr) => {
                    let data = match version.as_u64() {
                        1 => {
                            let function = try_s!(SWAP_CONTRACT_V1.function("erc20Payment"));
                            try_s!(function.encode_input(&[
                                Token::FixedBytes(id),
                        ...
                        2 => {
                            let function = try_s!(SWAP_CONTRACT_V2.function("erc20Payment"));
                            try_s!(function.encode_input(&[
                                Token::FixedBytes(id),
                                Token::Uint(U256::from(value)),
                                Token::Address(token_addr),
                    ...
                    let allowed = try_s!(coin.allowance(coin.swap_contract_address).compat().await);
                    if allowed < value {
                        let balance = try_s!(coin.my_balance().compat().await);
                        let _approved = try_s!(coin.approve(coin.swap_contract_address, balance).compat().await);
                    }
    ...
    fn spend_hash_time_locked_payment(
        ...
        let fut = async move {
            ...
            match version {
                1 => {
                    ...
                    match coin.coin_type {
                        EthCoinType::Eth => {
                            ...
                            let data = try_s!(spend_func.encode_input(&[
                                decoded[0].clone(),
                ...
                2 => {
                    let spend_func = try_s!(SWAP_CONTRACT_V2.function("receiverSpend"));
                    match coin.coin_type {
                        EthCoinType::Eth => {
    ...
    fn refund_hash_time_locked_payment(
        ...
        let fut = async move {
            ...
            match version {
                1 => {
                    ...
                    match coin.coin_type {
                        EthCoinType::Eth => {
                            ...
                            let data = try_s!(refund_func.encode_input(&[
                                decoded[0].clone(),
                                Token::Uint(value),
                                decoded[2].clone(),
                                Token::Address(Address::default()),
                ...
                2 => {
                    let refund_func = try_s!(SWAP_CONTRACT_V2.function("senderRefund"));
                    match coin.coin_type {
                        EthCoinType::Eth => {
                            let payment_func = try_s!(SWAP_CONTRACT_V2.function("ethPayment"));
                            let decoded = try_s!(payment_func.decode_input(&payment.data));
    ...
    fn my_balance(&self) -> Box<dyn Future<Item=U256, Error=String> + Send> {
        match self.coin_type {
            ...
            EthCoinType::Erc20(token_addr) => {
                ...
                let call_fut = self.call_request(token_addr, None, Some(data.into())).map_err(|e| ERRL!("{:?}", e));
    ...
    fn payment_sent_events(&self, from_block: u64) -> Box<dyn Future<Item=Vec<Log>, Error=String> + Send> {
        let contract_event = try_fus!(SWAP_CONTRACT_V2.event("PaymentSent"));
        let filter = FilterBuilder::default()
            .topics(Some(vec![contract_event.signature()]), None, None, None)
            .from_block(BlockNumber::Number(from_block))
    ...
    fn check_if_my_payment_sent(&self, time_lock: u32, secret_hash: &SecretHash, from_block: u64)
        ...
        let selfi = self.clone();
        let fut = async move {
            let status = try_s!(selfi.payment_status(selfi.swap_contract_address, &SWAP_CONTRACT_V2, Token::FixedBytes(id.clone())).compat().await);
...
impl Transaction for SignedEthTx {
    ...
    fn extract_secret(&self) -> Result<Vec<u8>, String> {
        let function = try_s!(SWAP_CONTRACT_V2.function("receiverSpend"));
