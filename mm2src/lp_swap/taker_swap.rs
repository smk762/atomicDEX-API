...
impl TakerSwap {
    ...
    pub fn new(
        ...
        TakerSwap {
            ...
            mutable: RwLock::new(TakerSwapMut {
                ...
                secret_hash: SecretHash::default(),
    ...
    pub async fn recover_funds(&self) -> Result<RecoveredSwap, String> {
        ...
        let taker_payment_lock = self.r().data.taker_payment_lock as u32;
...
pub fn migrate_taker_saved_swap(mut old_json: Json) -> Result<Json, String> {
    match old_json["events"].as_array_mut() {
        Some(events) => {
            for event in events.iter_mut() {
                match event["event"]["type"].as_str() {
                    Some("Negotiated") => {
                        let maker_pubkey: H264Json = try_s!(json::from_value(event["event"]["data"]["maker_pubkey"].clone()));
                        let ec_pubkey = EcPubkey {
                            curve_type: CurveType::SECP256K1,
                            bytes: maker_pubkey.0.to_vec(),
                        };
                        ...
                        let secret_hash: H160Json = try_s!(json::from_value(event["event"]["data"]["secret_hash"].clone()));
                        let secret_hash = SecretHash::from(secret_hash.0);
...
mod taker_swap_tests {
    ...
    fn get_migrated_swap(swap: &str) -> TakerSavedSwap {
        let old_json: Json = unwrap!(json::from_str(swap));
        let new_json = unwrap!(migrate_taker_saved_swap(old_json));
    ...
    fn test_recover_funds_taker_swap_maker_payment_spend_errored() {
        ...
        let taker_saved_swap: TakerSavedSwap = get_migrated_swap(taker_saved_json);
        let key_pair = unwrap!(ec_privkey_from_seed("spice describe gravity federal blast come thank unfair canal monkey style afraid"));
        let ctx = MmCtxBuilder::default().with_ec_privkey(key_pair).into_mm_arc();
        ...
        TestCoin::search_for_swap_tx_spend_other.mock_safe(|_, _, _, _, _, _| MockResult::Return(
        ...
        let actual = unwrap!(block_on(taker_swap.recover_funds()));
    ...
    fn test_recover_funds_taker_swap_taker_payment_errored_but_sent_and_spent_by_maker() {
        ...
        TestCoin::search_for_swap_tx_spend_my.mock_safe(|_, _, _, _, _, _| {
            ...
            let res = Some(FoundSwapTxSpend::Spent(tx.into()));
