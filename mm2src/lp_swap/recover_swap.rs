...
use crate::mm2::lp_swap::taker_swap::{maker_payment_wait, MakerNegotiationData, TakerSavedEvent, TakerSwapData,
...
fn recover_maker_swap(ctx: MmArc, taker_swap: TakerSavedSwap) -> RecoverSwapResult<MakerSavedSwap> {
...
/// Please note that this method ignores the following events:
/// * [`TakerSwapEvent::Started`] and [`TakerSwapEvent::Negotiated`]
...
/// * [`TakerSwapEvent::TakerPaymentSpent`], [`TakerSwapEvent::MakerPaymentSpent`], [`TakerSwapEvent::TakerPaymentRefunded`]
///   because we can't guarantee that `TakerPayment` is valid.
fn convert_taker_to_maker_events(event_it: impl Iterator<Item = TakerSavedEvent>) -> Vec<MakerSavedEvent> {
    ...
    for TakerSavedEvent { event, timestamp } in event_it {
        match event {
            ...
            TakerSwapEvent::TakerPaymentSent(tx_ident) => events.push(MakerSavedEvent {
                timestamp,
                event: MakerSwapEvent::TakerPaymentReceived(tx_ident),
            }),
            // We should ignore the `TakerPaymentSpent`, `MakerPaymentSpent`, `TakerPaymentRefunded` events
            // because we can't guarantee that `TakerPayment` is validated and confirmed.
            _ => (),
        }
    }
    events
...
fn recover_taker_swap(ctx: MmArc, maker_swap: MakerSavedSwap) -> RecoverSwapResult<TakerSavedSwap> {
    ...
    let taker_negotiated_event = TakerSwapEvent::Negotiated(MakerNegotiationData {
        ...
        secret_hash: started_event.secret_hash.or_mm_err(|| RecoverSwapError::NoSecretHash)?,
        maker_coin_swap_contract_addr: negotiated_event.maker_coin_swap_contract_addr,
        taker_coin_swap_contract_addr: negotiated_event.taker_coin_swap_contract_addr,
    });
    taker_swap.events.push(TakerSavedEvent {
    ...
    taker_swap.events.extend(convert_maker_to_taker_events(event_it));
...
/// Please note that this method ignores the following events:
/// * [`MakerSwapEvent::Started`] and [`MakerSwapEvent::Negotiated`]
...
/// * [`TakerSwapEvent::TakerPaymentReceived`], [`TakerSwapEvent::TakerPaymentSpent`], [`TakerSwapEvent::MakerPaymentRefunded`]
///   because we can't guarantee that `MakerPayment` is valid.
fn convert_maker_to_taker_events(event_it: impl Iterator<Item = MakerSavedEvent>) -> Vec<TakerSavedEvent> {
    let mut events = Vec::new();
    for MakerSavedEvent { event, timestamp } in event_it {
        match event {
            ...
            MakerSwapEvent::MakerPaymentSent(tx_ident) => events.push(TakerSavedEvent {
                timestamp,
                event: TakerSwapEvent::MakerPaymentReceived(tx_ident),
            }),
            // We should ignore the `TakerPaymentReceived`, `TakerPaymentSpent`, `MakerPaymentRefunded` events
            // because we can't guarantee that `MakerPayment` is validated and confirmed.
            _ => (),
        }
    }
    events
...
mod tests {
    ...
    fn test_recover_maker_swap() {
        ...
        let maker_actual_swap = recover_maker_swap(ctx, taker_saved_swap).expect("!recover_maker_swap");
    ...
    fn test_recover_taker_swap() {
        ...
        let taker_actual_swap = recover_taker_swap(ctx, maker_saved_swap).expect("!recover_maker_swap");
