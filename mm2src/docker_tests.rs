...
mod docker_tests {
    ...
    impl BchDockerOps {
        ...
        fn initialize_slp(&self) {
            ...
            let slp_genesis = TransactionOutput {
                ...
                script_pubkey: Builder::build_p2pkh(&self.coin.my_public_key().unwrap().address_hash()).to_bytes(),
            };
    ...
    fn test_utxo_merge() {
        ...
        thread::sleep(Duration::from_secs(2));
        let (unspents, _) = block_on(coin.list_unspent_ordered(&coin.as_ref().derivation_method.unwrap_iguana())).unwrap();
